...
const handleModalClick=(e,actionneur)=>{
  ...
  if (actionneur==='Registrer'|| true){
    UpdateProps('https://teamniintcheft.onrender.com/api/chantiers/majProps/'+ID,item) //Mise à jour des autres props (non nested props): id à determiner
    UpdateProps('https://teamniintcheft.onrender.com/api/chantiers/'+ID,item.dates) //Mise à jour des dates(props nested)
    for(let i=0;i<9;i++){
      ...
      if((file!==null && file!=='') ||testerSoustitres[i]!==''){//provisoire
        if(testerSoustitres[i]===''){alert('Les sous_titres des images déjà chargées sont obligatoires avant de les enregistrer !')}
            ...
            fetch('https://teamniintcheft.onrender.com/api/chantiers/nouveauChantier/'+ID+'/'+prop(i), { //Mise à jour des props nested (etat,rendu et programme) :id à determiner
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json'
              },
            ...
            fetch('https://teamniintcheft.onrender.com/uploadimage/'+stringFichier+'/'+nameInForm, {
                method: 'POST',
                body: formData,
                })
          ...
          if(file!==null || file!==''){
              ...
              UpdateProps('https://teamniintcheft.onrender.com/api/chantiers/nouveauChantier/'+ID+'/'+prop(i),imageData)
          }
          
        }}
      if(actionneur==='Valider' && Valid){
        Poster('https://teamniintcheft.onrender.com/api/chantiers/newChantier',{...item,id:idC})
        UpdateProps('https://teamniintcheft.onrender.com/api/chantiers/nouveauChantier',chantierAVide)
      }
 ...
 if(name===null){
    fetch('https://teamniintcheft.onrender.com/api/chantiers/getNewChantier/64ea4cd959615516c76ae0b2') 
      .then(response => response.json())
    ...
    fetch('https://teamniintcheft.onrender.com/api/chantiers/'+name) //+ID)
    .then(response => response.json())
